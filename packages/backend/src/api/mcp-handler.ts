...
import { z } from "zod";
import {
  searchProductsNaturalLanguage,
  validateShippingAddress,
} from "../somnia-agents";

/**
 * MCP Server Handler for Agent Checkout shopping agent
 */
...
async function initializeMCPServer(): Promise<McpServer> {
  ...
  const server = new McpServer({
    name: "agent-checkout-shopping-agent",
    version: "2.0.0",
  });
  ...
  server.tool(
    "search_products_nl",
    "Search store products using natural language (Somnia Agents)",
    {
      storeId: z.string().describe("The ID of the store"),
      query: z.string().describe("Natural language search query"),
    },
    async (args) => {
      return await handleSearchProductsNl(args as { storeId: string; query: string });
    }
  );
  ...
  server.tool(
    ...
    async (args) => {
      return await handleValidateShipping(args as { shippingAddress: any });
    }
  );
  ...
  server.tool(
    ...
    "Finalize checkout after user confirms payment (PHASE 2)",
    {
      ...
      paymentProof: z.object({
        txHash: z.string().describe("Somnia transaction hash"),
        network: z
          .string()
          .describe("Network (somnia-testnet or somnia-mainnet)"),
        chainId: z.number().describe("Chain ID (50312 for testnet)"),
    ...
    if (method === "initialize") {
      ...
      switch (toolName) {
        ...
        case "get_store_products":
          ...
          break;
        case "search_products_nl":
          toolResult = await handleSearchProductsNl(toolArgs);
          break;
        case "validate_shipping":
          toolResult = await handleValidateShipping(toolArgs);
          break;
...
async function handleSearchProductsNl(args: {
  storeId: string;
  query: string;
}): Promise<any> {
  const productsResult = await handleGetStoreProducts({ storeId: args.storeId });
  if (productsResult.error) return productsResult;

  const products = (productsResult.products || []).map((p: any) => ({
    id: p.id,
    title: p.title,
    price: p.price,
  ...
  const matches = await searchProductsNaturalLanguage(args.query, products);
  return { success: true, query: args.query, matches };
}

async function handleValidateShipping(args: {
  shippingAddress: {
    address1: string;
    city: string;
    state?: string;
  ...
  const result = await validateShippingAddress(args.shippingAddress);
